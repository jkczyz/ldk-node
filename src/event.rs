...
pub enum Event {
	...
	/// mid-negotiation) is retried automatically while the node runs; this event is emitted only
	/// once the splice is given up on.
	...
	L::Target: LdkLogger,
{
	pub fn new(
		...
		splice_retrier: Arc<SpliceRetrier>, runtime: Arc<Runtime>, logger: L, config: Arc<Config>,
	) -> Self {
		Self {
			event_queue,
			wallet,
	...
	pub async fn handle_event(&self, event: LdkEvent) -> Result<(), ReplayEvent> {
		match event {
			...
			LdkEvent::SpliceNegotiationFailed {
				...
				// A user-initiated splice is retried automatically while the node runs; surface
				// the failure only once it is given up on.
				let surface = self
					.splice_retrier
