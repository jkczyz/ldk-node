...
pub(crate) enum PendingPaymentDetails {
	...
	/// entry points and the splice tracker — persisted at splice initiation, carried over from a
	/// [`PendingSplice`] record when the payment is promoted, and cleared once the splice locks
	/// or its failure is surfaced. A splice uses all of them; the fields do not partition by
	/// payment type.
	///
	/// [`PendingSplice`]: Self::PendingSplice
	Tracked {
	...
	fn update(&mut self, update: Self::Update) -> bool {
		match self {
			...
			Self::Tracked { details, conflicting_txids, candidates, splice_intent } => {
				...
				// Each classify passes the candidate history as of its own broadcast, so a
				// non-empty update replaces the stored list. An empty update (e.g. a non-funding
				// payment) leaves it untouched — as does an update missing a stored candidate:
				// the history only ever grows, so such an update was built before that candidate
				// existed (a classification retry running after a newer round classified) and
				// replacing would orphan the newer round's transactions.
				...
				if !update.candidates.is_empty()
					...
					&& candidates.iter().all(extends_history)
				{
					*candidates = update.candidates;
					updated = true;
				}
	...
	fn from(value: &PendingPaymentDetails) -> Self {
		match value {
			...
			PendingPaymentDetails::Tracked {
				...
				// splice tracker, never by a payment-tracking merge. Emitting the current value here
				// would let an `insert_or_update` of a payment record (e.g. from wallet sync, built
				// without an intent) clobber a live intent to `None`.
				let _ = splice_intent;
				Self {
					id: details.id,
...
mod tests {
	...
	fn candidate_history_never_shrinks() {
		...
		let stored_candidates = |pending: &PendingPaymentDetails| match pending {
			PendingPaymentDetails::Tracked { candidates, .. } => candidates.clone(),
			pending => panic!("unexpected variant {:?}", pending),
		};
		...
		assert_eq!(stored_candidates(&pending), history);
		...
		assert_eq!(stored_candidates(&pending), extended);
	}
	...
	fn tracked_payment_round_trips() {
		// The `PendingSplice` variant round-trips in `pending_splice_round_trips`; here we cover the
		// `Tracked` variant and its enum discriminant.
		let payment_id = PaymentId([7u8; 32]);
